...
class Go2Env:
    def __init__(self, num_envs, env_cfg, obs_cfg, reward_cfg, command_cfg, show_viewer=False, eval_mode=False,device="cuda"):
        ...
        self.waypoints = torch.tensor([
            [[0.0, 0.0, 0.3], [1.0, 1.0, 0.3], [2.0, 2.0, 0.3]]
            ...
            for env_id in range(self.num_envs):
                for wp in self.waypoints[env_id]:
                    marker = self.scene.add_entity(
                        gs.morphs.Sphere(radius=0.05, pos=wp.cpu().numpy())
                    )
    ...
    def _resample_commands(self, envs_idx):
        ...
        lin_vel_x = torch.clamp(direction[:, 0], *self.command_cfg["lin_vel_x_range"])
        lin_vel_y = torch.clamp(direction[:, 1], *self.command_cfg["lin_vel_y_range"])
        ...
        ang_vel = torch.clamp(angle_to_waypoint - current_heading, *self.command_cfg["ang_vel_range"])
